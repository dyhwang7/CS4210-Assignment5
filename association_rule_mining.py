...
# SPECIFICATION: find rules that satisfy support = 0.2 and confidence = 0.6
...
for index, row in df.iterrows():
    labels = {}
    input_set = set(row)
    not_included_set = itemset - input_set

    for i in row:
        if i in itemset:
            labels[i] = 1

    for i in list(not_included_set):
        labels[i] = 0
...
for index, row in rules.iterrows():
    ...
    print("{} -> {}".format(', '.join(ant), ', '.join(conseq)))
    print("Support: {}".format(row['support']))
    print("Confidence: {}".format(row['confidence']))
    print("Prior: {}".format(prior))
    print("Gain in Confidence: {}".format(100*(confidence-prior)/prior))
    print()
#Meat, Cheese -> Eggs
